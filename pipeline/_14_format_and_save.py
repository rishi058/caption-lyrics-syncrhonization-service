...
def format_and_save(segment_data: list[dict], media_path: str, output_dir: str, audio_duration: float, language: str, devanagari_output: bool) -> None:
    
    if language == "hi":
        segment_data = format_segment_for_hindi(segment_data, devanagari_output)
    ...
    save_data(segment_data, f"{media_name}_raw.json", output_dir)
        ...
        for chunk in seg["chunks"]:
            for word in chunk.get("words", []):
                final_data.append({
                    ...
                    "timestampMs": int(word.get("start", 0) * 1000),
                    "confidence": round(word.get("confidence", word.get("score", 0.0)), 6)
                })
    ...
    final_data.append({
        ...
        "timestampMs": final_data[-1]["endMs"] + 1, "confidence": 1,
    })
